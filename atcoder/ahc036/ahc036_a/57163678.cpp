// Date: Mon, 26 Aug 2024 18:27:24 +0000
...
struct Ans {
  ...
  void add_move(ll v) { op.emplace_back("m " + to_string(v)); }

  void add_signal(ll length, ll a, ll b) {
    op.emplace_back("s " + to_string(length) + " " + to_string(a) + " " +
                    to_string(b));
    score++;
  }
};
...
struct Option {
  vector<vector<Edge>> (*create_edge)(ll N, ll LB,
                                      vector<pair<ll, ll>> &default_edge);
...
Ans solve(ll N, ll M, ll T, ll LA, ll LB, vector<pair<ll, ll>> &default_edge,
  ...
  Ans ans;
  vector<vector<Edge>> edge = option.create_edge(N, LB, default_edge);
...
int main() {
  ...
  Option opt1 = {
      .create_edge = [](ll N, ll LB, vector<pair<ll, ll>> &default_edge)
          -> vector<vector<Edge>> {
        vector<vector<Edge>> edge(N);
        for (auto [u, v] : default_edge) {
          edge[u].push_back({v, 100});
          edge[v].push_back({u, 100});
        }
        return edge;
      },
  ...
  Option opt2 = {
      .create_edge = [](ll N, ll LB, vector<pair<ll, ll>> &default_edge)
        ...
        vector<vector<Edge>> edge(N);
        for (auto [u, v] : default_edge) {
          edge[u].push_back({v, 600 * LB});
          edge[v].push_back({u, 600 * LB});
        }
        return edge;
      },
      .update_cost = [](ll cost, ll LB, ll T, ll target) -> ll {
        return max(cost - (2 * LB * (T - target)), 1);
      },
  };
  ...
  if (now_time < 1500000) {
    ...
    Ans ans3 = solve(N, M, T, LA, LB, default_edge, t, opt3);
    ll score = min({ans1.score, ans2.score, ans3.score});
    if (ans1.score == score) {
      cout << "# select ans1" << endl;
      ans1.output();
    } else if (ans2.score == score) {
      cout << "# select ans2" << endl;
      ans2.output();
    } else if (ans3.score == score) {
      cout << "# select ans3" << endl;
      ans3.output();
    }
  } else {
    ll score = min({ans1.score, ans2.score});
    if (ans1.score == score) {
      cout << "# select ans1" << endl;
      ans1.output();
    } else if (ans2.score == score) {
