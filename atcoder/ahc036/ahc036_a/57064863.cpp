// Date: Sat, 24 Aug 2024 12:41:27 +0000
...
int main() {
  ...
  auto calc_path = [&]() -> pair<vector<ll>, vector<ll>> {
    ...
    for (ll i = 0; i < T - 1; i++) {
      ...
      for (ll left = 0; left < path.size(); left++) {
        for (ll length = 1; length <= LB && left + length - 1 < path.size();
             length++) {
          set<ll> sub_path(path.begin() + left, path.begin() + left + length);
          count[sub_path]++;
        }
      }
    }
    vector<priority_queue<pair<ll, set<ll>>>> que(LB);
    ...
    vector<ll> A;
    for (ll l = LB - 1; 0 <= l; l--) {
      priority_queue<pair<ll, set<ll>>> tmp = que[l];
      while (0 < tmp.size()) {
        auto [c, v] = tmp.top();
        tmp.pop();
        bool ok = true;
