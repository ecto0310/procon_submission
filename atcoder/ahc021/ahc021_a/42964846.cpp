// Date: Sun, 25 Jun 2023 14:28:54 +0000
  ...
  i64 update_score() {
    ...
    for (i64 i = 0; i < N - 1; i++) {
      for (i64 j = 0; j <= i; j++) {
        ...
        score += (step - i) * (step - i) * this->b[i][j].first * this->b[i][j].first;
      }
    }
  ...
  for (i64 K = 0; K < 10000; K++) {
    while (0 < que[K % N].size()) {
      ...
      if (step == 1) {
        ...
        if (ind == (i64)putable.size()) {
          ind--;
        } else if (0 <= ind - 1 &&
                   abs(putable[ind - 1] - y) < abs(putable[ind] - y)) {
          ind--;
        }
