// Date: Tue, 03 Dec 2024 14:00:36 +0000
...
fn main() {
    ...
    let time_limit_sep = Duration::from_millis((2800 / input.T).try_into().unwrap());
    let time_limit = Duration::from_millis(1400);
    ...
    pub fn new(input: &Input, rng: &mut ThreadRng, d: char) -> Self {
        ...
        if d == 'U' {
            for idx in 0..(input.N) {
                ...
                eprintln!("# {} {}", now + w, now + w < wh_threshold);
            ...
            for idx in 0..(input.N) {
                ...
                eprintln!("# {} {}", now + h, now + h < wh_threshold);
        ...
        let sn = input.N - last_start;
        let mut best_bit = 0;
        let mut best_score = 1000000000000i64;
        for bit in 0..(1 << sn) {
            (0..sn).for_each(|x| {
                cmds[last_start + x].r = bit & (1 << x) != 0;
            });
        ...
        println!("# {} {:b}", sn, best_bit);
        (0..sn).for_each(|x| {
            cmds[last_start + x].r = best_bit & (1 << x) != 0;
        });
