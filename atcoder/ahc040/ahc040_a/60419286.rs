// Date: Tue, 03 Dec 2024 15:00:40 +0000
...
fn main() {
    ...
    let time_limit_sep = Duration::from_millis((2500 / input.T).try_into().unwrap());
    let time_limit = Duration::from_millis(1250);
...
impl Output {
    pub fn new(input: &Input, rng: &mut ThreadRng, d: char) -> Self {
        ...
        while start < input.N {
            ...
            let sn = (last_start - start).min(10);
            let mut best_bit = 0;
            let mut best_score = 1000000000000i64;
            for bit in 0..(1 << sn) {
                (0..sn).for_each(|x| {
                    cmds[start + x].r = bit & (1 << x) != 0;
                });
