// Date: Mon, 09 Dec 2024 04:22:37 +0000
...
fn median(mut numbers: Vec<i64>) -> i32 {
    // let mut sort_numbers = numbers.clone();
    numbers.sort();
    let len = numbers.len();
    let target = len / 2;
    let median = match len % 2 {
        0 => (numbers[target] + numbers[target - 1]) / 2,
        1 => numbers[target],
        _ => 0,
    };
    median as i32
...
fn average(mut numbers: Vec<i64>) -> i32 {
    // let mut sort_numbers = numbers.clone();
    numbers.sort();
    ...
    let s = numbers.len() as i64;
    let sum = numbers.iter().fold(0i64, |sum, i| sum + i);
    (sum / s).try_into().unwrap()
}

fn sigma(numbers: &Vec<i64>) -> i32 {
    let s = numbers.len() as i64;
    let ave = average(numbers.clone()) as i64;

    let sig = (numbers
        .iter()
        .fold(0i64, |sum, i| sum + (i - ave) * (i - ave))
        / (s - 1))
        .sqrt();
    sig as i32
}

fn select(numbers: Vec<i64>, input: &Input) -> i32 {
    // let s = numbers.len();
    // let sig = sigma(&numbers);
    // if sig < input.sigma || s < 3 {
    //     average(numbers)
    // } else {
    //     let numbers = numbers[1..s - 1].to_vec();
    //     average(numbers)
    // }
...
impl Input {
    ...
    pub fn fix<R: BufRead>(&mut self, source: &mut LineSource<R>) {
        let challenge_query = 10;
        if self.T < self.N + challenge_query {
            println!("# No fix box size");
            return;
        }
        let mut data = vec![vec![]; self.N];
        let try_count = ((self.T - challenge_query) / self.N).min(2);
        let mod_try_count = self.T - challenge_query - try_count * self.N;
        for idx in 0..(self.N) {
            data[idx].push((self.wh[idx].0 as i64, self.wh[idx].1 as i64));
            let now_try_count = try_count + if idx < mod_try_count { 1 } else { 0 };
            println!("# Fix box {} size  {} query", try_count, now_try_count);
            for _ in 0..try_count {
                let output = Output {
                    cmds: vec![Cmd::new(idx, false, 'U', -1)],
                };
                let (w, h, _) = output.submit(self, source);
                data[idx].push((w as i64, h as i64));
            }
        }
        let sigs = data
            .iter()
            .enumerate()
            .map(|(idx, d)| {
                let ws = d.iter().map(|i| i.0).collect::<Vec<i64>>();
                let hs = d.iter().map(|i| i.1).collect::<Vec<i64>>();
                (sigma(&ws).max(sigma(&hs)), idx)
            })
            .collect::<Vec<(i32, usize)>>();

        let mut que = BinaryHeap::from(sigs);
        for _ in 0..mod_try_count {
            let sig = que.pop().unwrap();
            eprintln!("{:?}", sig);
            let (_, idx) = sig;
            let output = Output {
                cmds: vec![Cmd::new(idx, false, 'U', -1)],
            ...
            let (w, h, _) = output.submit(self, source);
            data[idx].push((w as i64, h as i64));
            let ws = data[idx].iter().map(|i| i.0).collect::<Vec<i64>>();
            let hs = data[idx].iter().map(|i| i.1).collect::<Vec<i64>>();
            que.push((sigma(&ws).max(sigma(&hs)), idx));
        }
        for idx in 0..(self.N) {
            let ws = data[idx].iter().map(|i| i.0).collect::<Vec<i64>>();
            let hs = data[idx].iter().map(|i| i.1).collect::<Vec<i64>>();
            self.wh[idx] = (select(ws, &self), select(hs, &self));
        }
        self.T = challenge_query;
    }
}
