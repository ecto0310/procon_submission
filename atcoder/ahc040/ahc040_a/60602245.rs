// Date: Mon, 09 Dec 2024 03:48:17 +0000
...
impl Input {
    ...
    pub fn fix<R: BufRead>(&mut self, source: &mut LineSource<R>) {
        ...
        let try_count = (self.T - challenge_query) / self.N;
        let mod_try_count = (self.T - challenge_query) % self.N;
        for idx in 0..(self.N) {
        ...
        sigs.sort_by(|a, b| b.cmp(&a));
        for idx in 0..mod_try_count {
            eprintln!("{:?}", sigs[idx]);
            let target = sigs[idx].1;
            let output = Output {
                cmds: vec![Cmd::new(target, false, 'U', -1)],
            };
            let (w, h, _) = output.submit(self, source);
            data[target].push((w as i64, h as i64));
        }
        for idx in 0..(self.N) {
