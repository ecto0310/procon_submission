// Date: Sat, 22 Feb 2025 17:41:43 +0000
    ...
    def solve(self) -> Result:
        ...
        for person_idx in range(0, self.M):
            ...
            if len(self.actions) + dist + 1 <= self.T and \
                ...
                if len(select_in_count) <= len(in_count) and select_benefit < benefit:
        ...
        first_person = select_person_idx
        for person_idx in select_in_count:
            if distance(self.home[person_idx], self.home[first_person]) <= 2:
                self.home[person_idx] = self.home[first_person]
                # if distance(self.workplace[first_person], self.workplace[person_idx]) < distance(self.home[person_idx], self.workplace[person_idx]):
                #     self.home[person_idx] = self.workplace[first_person]
            if distance(self.home[person_idx], self.workplace[first_person]) <= 2:
                self.home[person_idx] = self.workplace[first_person]
                # if distance(self.home[first_person], self.workplace[person_idx]) < distance(self.home[person_idx], self.workplace[person_idx]):
                #     self.home[person_idx] = self.home[first_person]
            if distance(self.workplace[person_idx], self.home[first_person]) <= 2:
                self.workplace[person_idx] = self.home[first_person]
                # if distance(self.workplace[first_person], self.home[person_idx]) < distance(self.home[person_idx], self.workplace[person_idx]):
                #     self.workplace[person_idx] = self.workplace[first_person]
            if distance(self.workplace[person_idx], self.workplace[first_person]) <= 2:
                self.workplace[person_idx] = self.workplace[first_person]
                # if distance(self.home[first_person], self.home[person_idx]) < distance(self.home[person_idx], self.workplace[person_idx]):
                #     self.workplace[person_idx] = self.home[first_person]
            if not self.check_build(person_idx):
                self.workplace[person_idx], self.home[person_idx] = self.home[person_idx], self.workplace[person_idx]

        select_in_count = list(filter(self.check_build, select_in_count))
        select_in_count = list(filter(lambda x: not (self.home[x] == self.home[first_person] and self.workplace[x] == self.workplace[first_person]) or (
            self.home[x] == self.workplace[first_person] and self.home[x] == self.home[first_person]), select_in_count))
