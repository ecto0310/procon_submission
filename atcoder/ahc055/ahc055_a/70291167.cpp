// Date: Sun, 19 Oct 2025 11:01:52 +0000
...
Output solve(const Input& input) {
    ...
    // 初期解：攻撃力の高い武器から順番に効率的な宝箱を攻撃
    vector<int> chest_order;
    vector<bool> chest_used(N, false);
    vector<bool> weapon_used(N, false);
    
    // 武器を攻撃力の大きい順にソート
    vector<pair<int, int>> weapons; // (攻撃力, 武器番号)
    for (int w = 0; w < N; w++) {
        int max_attack = *max_element(A[w].begin(), A[w].end());
        weapons.push_back({max_attack, w});
    }
    sort(weapons.rbegin(), weapons.rend());
    
    // 各武器について、その武器で最も効果的な宝箱を攻撃し、効率が悪くなるまで続ける
    for (auto& weapon_info : weapons) {
        int weapon = weapon_info.second;
        if (weapon_used[weapon]) continue;
        
        // まず、この武器の宝箱を開ける（武器を獲得する）
        if (!chest_used[weapon]) {
            chest_order.push_back(weapon);
            chest_used[weapon] = true;
        }
        
        bool continue_attacking = true;
        while (continue_attacking) {
            ...
            for (int c = 0; c < N; c++) {
                ...
                if (A[weapon][c] > best_damage) {
                    best_damage = A[weapon][c];
                    best_chest = c;
                }
            }
            
            // 攻撃力が1以下または他の未使用武器の方が効率的な場合は停止
            if (best_chest == -1 || best_damage <= 1) {
                continue_attacking = false;
                cerr << "best_chest == -1 || best_damage <= 1" << endl;
            } else {
                // 他の未使用武器の方が効率的かチェック
                bool other_weapon_better = false;
                for (auto& other_weapon_info : weapons) {
                    int other_weapon = other_weapon_info.second;
                    if (weapon_used[other_weapon]) continue;
                    
                    for (int c = 0; c < N; c++) {
                        if (chest_used[c]) continue;
                        if (A[other_weapon][c] > best_damage) {
                            other_weapon_better = true;
                            break;
                        }
                    }
                    if (other_weapon_better) break;
                }
                
                if (other_weapon_better) {
                    continue_attacking = false;
                    cerr << "other_weapon_better: " << other_weapon_better << endl;
                } else {
                    // この武器で最も効果的な宝箱を攻撃
                    chest_order.push_back(best_chest);
                    cerr << "weapon: " << weapon << ", chest: " << best_chest << endl;
                    chest_used[best_chest] = true;
                }
            }
        }
        
        weapon_used[weapon] = true;
    }
    
    // まだ使われていない宝箱があれば追加
    for (int c = 0; c < N; c++) {
        if (!chest_used[c]) {
            chest_order.push_back(c);
        }
    }
