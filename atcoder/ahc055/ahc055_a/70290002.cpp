// Date: Sun, 19 Oct 2025 10:28:01 +0000
...
int evaluate(const Input& input, const vector<int>& order) {
    ...
    int total_attacks = 0;
        ...
        for (int chest : order) {
            while (current_hardness[chest] > 0) {
                int selected_weapon = select_weapon(current_hardness[chest], current_durability, weapon_available, A[chest]);
                if (selected_weapon != -1) {
                    // 武器で攻撃
                    current_hardness[chest] -= A[selected_weapon][chest];
                    current_durability[selected_weapon]--;
                    total_attacks++;
                } else {
                    // 素手で攻撃
                    current_hardness[chest]--;
                    total_attacks++;
                }
            }
...
Output solve(const Input& input) {
    ...
    // 初期解：宝箱の順序をランダムに
    vector<int> chest_order(N);
    iota(chest_order.begin(), chest_order.end(), 0);
    shuffle(chest_order.begin(), chest_order.end(), gen);
