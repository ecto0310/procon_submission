// Date: Wed, 03 Nov 2021 18:50:52 +0000
...
void solve(i64 day)
{
    ...
    {
        for (i64 i = 0; i < task_queue.size(); i++)
        {
            i64 best_member = -1, estimation_day = 1e9;
            for (i64 j = 0; j < member_queue.size(); j++)
            {
                i64 sum_day = 0;
                for (int k = 0; k < K; k++)
                {
                    sum_day += max(0LL, task[task_queue[i]].d[k] - member[member_queue[j]].s[k]);
                }
                if (sum_day < estimation_day)
                {
                    estimation_day = sum_day;
                    best_member = j;
                }
            }
            if (best_member != -1)
            {
                allocation.push_back({member_queue[best_member], task_queue[i]});
                member[member_queue[best_member]].task = task_queue[i];
                member[member_queue[best_member]].start_day = day;
                task[task_queue[i]].finish = true;
                member_queue.erase(member_queue.begin() + best_member);
            }
        }
    }
...
bool update(i64 day)
{
    ...
    for (i64 i = 0; i < n; i++)
    {
        ...
        if (task_day == 1)
        {
            for (i64 j = 0; j < K; j++)
            {
                member[t].min_s[j] = min(member[t].min_s[j], task[j].d[j]);
                member[t].s[j] = max(member[t].s[j], member[t].min_s[j]);
            }
        }
        member[t].instability = 0;
        int sum_d = accumulate(task[finish_task].d.begin(), task[finish_task].d.end(), 0LL);
        for (i64 j = 0; j < K; j++)
        {
            i64 estimation_d = max(member[t].min_s[j], (task[finish_task].d[j] * task_day + sum_d - 1) / sum_d);
            member[t].instability += abs(member[t].s[j] - estimation_d);
            member[t].s[j] = (member[t].s[j] + estimation_d) / 2;
        }
        member[t].task = -1;
    }
    return true;
