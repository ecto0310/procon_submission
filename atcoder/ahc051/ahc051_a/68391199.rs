// Date: Sun, 10 Aug 2025 10:41:51 +0000
...
impl Graph {
    ...
    pub fn build_graph(&mut self, input: &Input) {
        ...
        for (i, (separator_idx, separator_node)) in center_band_separator.iter().enumerate().rev() {
            ...
            for (disposal_idx, disposal_node) in &disposal_nodes {
                ...
                if can_connect {
                    eprintln!("can_connect: {}, i: {}", can_connect, i);
    ...
    pub fn optimize_graph(&mut self, input: &Input) {
        ...
        eprintln!("Installed separators: {:?}", installed_separators);
        ...
        if !current_error.is_empty() {
            eprintln!("Initial score error: {}", current_error);
        }
        
        let mut best_output = current_output.clone();
        let mut best_score = current_score;
        
        eprintln!("Initial score: {}", current_score);
        eprintln!("Initial disposal types: {:?}", current_output.ds);
        eprintln!("Initial separator info: {:?}", current_output.cs.iter().take(5).collect::<Vec<_>>());
        ...
        while start_time.elapsed().as_secs_f64() < time_limit {
            ...
            if !new_error.is_empty() {
                eprintln!("Score error at iteration {}: {}", iteration, new_error);
            ...
            if accepted {
                ...
                if current_score < best_score {
                    ...
                    eprintln!("New best score: {} (iteration: {})", best_score, iteration);
                }
            }
            
            // 定期的にデバッグ情報を出力
            if iteration % 1000 == 0 {
                eprintln!("Iteration: {}, Current score: {}, Best score: {}, Temp: {:.2}, Accept rate: {:.2}%, Change: {}", 
                    iteration, current_score, best_score, temp, 
                    (accepted_count as f64 / iteration as f64) * 100.0, change_info);
            }
        }
        ...
        eprintln!("Optimization completed: {} iterations, accepted: {}, final score: {}, best score: {}", 
            iteration, accepted_count, current_score, best_score);
        eprintln!("Final disposal types: {:?}", best_output.ds);
    }
    ...
    fn to_score_output(&self, score_input: &ScoreInput) -> ScoreOutput {
        ...
        eprintln!("to_score_output: s={}, ds={:?}, cs={:?}", s, ds, cs);
