// Date: Sun, 10 Aug 2025 19:02:20 +0000
...
// 交差判定を省略した最適化版スコア計算関数
fn compute_score_optimized(input: &ScoreInput, out: &ScoreOutput) -> (i64, String) {
    let mut probs = vec![vec![0.0; input.N]; input.N + input.M];
    let mut tmp = out.ds.clone();
    tmp.sort();
    tmp.dedup();
    if tmp.len() != input.N {
        return (0, format!("Duplicated d"));
    }
    if out.s >= input.N && out.cs[out.s - input.N].0 == !0 {
        return (0, format!("Invalid s {} {}", out.s, out.cs[out.s - input.N].0));
    }
    ...
    for i in 0..input.M {
        ...
        if out.cs[i].1 >= input.N && out.cs[out.cs[i].1 - input.N].0 == !0 {
            return (0, format!("Invalid v1 in the {}-th position", i));
        }
        if out.cs[i].2 >= input.N && out.cs[out.cs[i].2 - input.N].0 == !0 {
            return (0, format!("Invalid v2 in the {}-th position", i));
        }
        g[input.N + i].push(out.cs[i].1);
        g[input.N + i].push(out.cs[i].2);
    }
    
    // 交差判定を省略（optimize_graph内のグラフは常に交差しない）
    
    let Some(order) = topological_sort(&g) else {
        return (0, "Graph contains a cycle".to_owned());
    };
    for i in 0..input.N {
        probs[out.s][i] = 1.0;
    }
...
impl Graph {
    ...
    pub fn optimize_graph(&mut self, input: &Input) {
        ...
        let (mut current_score, current_error) = compute_score_optimized(&score_input, &current_output);
        ...
        while start_time.elapsed().as_secs_f64() < time_limit {
            ...
            let change_type = rng.gen_range(0..3); // 0: 処理装置の種類変更, 1: 分別器の種類変更， 2: 分岐器の出口の接続順の入れ替え
            ...
            match change_type {
                ...
                2 => {
                    ...
                    if !installed_separators.is_empty() {
                        let separator_idx = installed_separators[rng.gen_range(0..installed_separators.len())];
                        let old_exit1 = new_output.cs[separator_idx].1;
                        let old_exit2 = new_output.cs[separator_idx].2;
                        // 出口1と出口2を入れ替え
                        new_output.cs[separator_idx].1 = old_exit2;
                        new_output.cs[separator_idx].2 = old_exit1;
                        change_info = format!("Swap separator {} exit1 and exit2", separator_idx);
                    }
                }
                _ => unreachable!(),
            }
            ...
            // 新しい解のスコアを計算
            let (new_score, new_error) = compute_score_optimized(&score_input, &new_output);
    ...
    fn apply_score_output(&mut self, output: &ScoreOutput) {
        ...
        for i in 0..output.cs.len() {
            ...
            if output.cs[i].0 != !0 {
                ...
                // 接続も更新（出口の順序変更を反映）
                separator_node.connections.clear();
                separator_node.connections.push(output.cs[i].1 + 1); // +1はインデックス調整
                separator_node.connections.push(output.cs[i].2 + 1); // +1はインデックス調整
            }
        }
    }
}
