// Date: Sun, 11 Jun 2023 07:54:10 +0000
...
double rand01() { return (xorshift() + 0.5) * (1.0 / UINT_MAX); }
...
void solve(int N, int M, int K, vector<Sender> &senders,
  ...
  Ans ans = final_ans;
  i64 final_score = get_score(N, M, K, senders, edge_infos, houses, final_ans);
  ...
  for (;; loop++) {
    ...
    if (0.5 < rand01()) {
      ...
      int s = rand() % M;
      ans.B[s] = !ans.B[s];
      i64 tmp_score = get_score(N, M, K, senders, edge_infos, houses, ans);
      ...
      // いずれかの点をパワーを乱数値増加減少
      int s = rand() % N;
      int dec_p = rand() % 2000 - 500;
      int backup_p = ans.P[s];
      ans.P[s] = max(0, ans.P[s] - dec_p);
      i64 tmp_score = get_score(N, M, K, senders, edge_infos, houses, ans);
      double accept = exp((tmp_score - score) / temp);
      if (accept > rand01()) {
        score = tmp_score;
