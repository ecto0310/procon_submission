// Date: Wed, 02 Oct 2024 12:08:36 +0000
// Language: C++17
#include <bits/stdc++.h>
using namespace std;
// #include <atcoder/all>
...
/* vector */
template <class T>
T vmax(vector<T> &array) {
    T ret = array[0];
    for (T a : array) chmax(ret, a);
    return ret;
}
template <class T>
T vmin(vector<T> &array) {
    T ret = array[0];
...
template <class T>
void list_set(vector<T> &array) {
    sort(all(array));
    array.erase(unique(all(array)), array.end());
}
template <class T>
int bisect_left(vector<T> &array, T key) {
    return lower_bound(all(array), key) - array.begin();
}
template <class T>
int bisect_right(vector<T> &array, T key) {
...
#define VEC(type, name, size) \
    ...
    IN(name)
#define VECS(type, name, size)   \
    vector<type> name(size + 1); \
    for (int i = 1; i <= size; i++) scan(name[i])
...
template <class Head, class... Tail>
void FLASH(const Head &head, const Tail &...tail) {
    output(head);
    if (sizeof...(tail)) cout << " ";
    FLASH(tail...);
}
...
bool solve() {
    ...
    for (ll i = 0; i < n; i++) {
        a_sum += a[i];
        b_sum += b[i];
        if (status == 0 && a_sum < b_sum) {
            ans++;
        }
        if (status == 1 && b_sum < a_sum) {
            ans++;
        }
        if (a_sum != b_sum) {
            status = (b_sum < a_sum ? 0 : 1);
        }
    }
    cout << ans << endl;
    return true;
